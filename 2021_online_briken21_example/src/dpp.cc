...
void dpp::fast_maxADC(UShort_t&adcpos,UShort_t&adc,UShort_t* Waveline,Int_t nsamples,Int_t start) //work only for positive pulse
{
    adc = 0;
    adcpos = 0;
...
double dpp::fast_baseline(UShort_t* Waveline,Int_t nsamples)
{
  double sum=0;
  for (int i=0;i<nsamples;i++)
      sum+=(double)Waveline[i];
