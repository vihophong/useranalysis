...
public:
    ...
    void DecodeSamplesRaw(int* words,int ipos){
        //int pos = ipos;
        int nch_enable = 0;
        int chmap[16];
        for (int i=0;i<16;i++){
            if (((channel_mask>>i)&0x1)==0) continue;
            chmap[nch_enable] =i;
            nch_enable++;
        }
        int nwords_ch = (event_size - 4)/nch_enable;
        if ((event_size - 4)%nch_enable!=0){
            cout<<"DecodeSamplesRaw: Something wrong!"<<(event_size - 4)<<"\t"<<nch_enable<<endl;
        }
        int pos = ipos;
        for (int i=0;i<nch_enable;i++){
            NIGIRIHit* chdata=new NIGIRIHit;
            chdata->ch = chmap[i];
            chdata->nsample = nwords_ch*2;
            chdata->ts = ts;
            for (int j=0;j<nwords_ch;j++){//500 sample
                chdata->pulse.push_back((words[pos]&0x3FFF));
                chdata->pulse.push_back((words[pos]&0x3FFF0000)>>16);
                pos++;
            }
            AddHit(chdata);
        }
